...
אפליקציית **PWA** להורדת אודיו מיוטיוב כ‑MP3: ממשק מותאם מובייל, עם שדה קישור **ושיתוף ישיר מיוטיוב** לאפליקציה.
אין מה להגדיר — האפליקציה מוצאת את שרת ההמרה שלה לבד.
...
- **חיפוש בתוך האפליקציה** — הקלדת מילים במקום קישור מחזירה תוצאות עם תמונה, ערוץ ואורך
- **חיתוך קטע** — שני מחוונים על אורך הסרטון; החיתוך נעשה באותו מעבר של ffmpeg
- **עריכת שם הקובץ** — נשלח לשרת ונכנס גם לתגית הכותרת
- **תור הורדות** — מוסיפים כמה שירים והם מומרים בזה אחר זה
...
### שחרור גרסה חדשה של הממשק

קבצי הקוד לא נושאים חתימה בשם, אז לגרסה יש תג אחד שמופיע בשלושה מקומות וחייב להישאר זהה:
`BUILD` ב‑`web/sw.js`, `BUILD` ב‑`web/js/app.js`, וה‑`?v=` על התגיות ב‑`web/index.html`.
מעלים את שלושתם יחד — אחרת דפדפן שכבר ביקר באתר ימשיך להריץ את הגרסה הישנה.
מספר הגרסה מוצג בקטן בתחתית המסך הריק, כדי שאפשר יהיה לדעת מה בעצם רץ.
